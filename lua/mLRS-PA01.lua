...
----------------------------------------------------------------------
local edgeX = 10
local fontSizeSML = 16
local fontSize = 21
local fontSizeMID = 30
...
local function setupScreen()
    ...
    page_N1 = (LCD_H-titleHeight)/fontSize-2
    page_N = page_N1*2
end    
...
local function drawPopup()
    lcd.drawFilledRectangle(LCD_W/6-2, LCD_H/4-2, LCD_W*2/3+4, LCD_H/4+4, TEXT_COLOR) --TITLE_BGCOLOR)
    lcd.drawFilledRectangle(LCD_W/6, LCD_H/4, LCD_W*2/3, LCD_H/4, TITLE_BGCOLOR) --TEXT_BGCOLOR) --TITLE_BGCOLOR)

    local i = string.find(popup_text, "\n")
    local attr = MENU_TITLE_COLOR+MIDSIZE+CENTER
    if i == nil then
        lcd.drawText(LCD_W/2, LCD_H/4+fontSizeMID/2, popup_text, attr)
    else
        local t1 = string.sub(popup_text, 1,i-1)
        local t2 = string.sub(popup_text, i+1)
        lcd.drawText(LCD_W/2, LCD_H/4+5, t1, attr)
        lcd.drawText(LCD_W/2, LCD_H/4+fontSizeMID+5, t2, attr)
    end
end
...
local function drawPageEdit(page_str)
    ...
    for pidx = 2, #DEVICE_PARAM_LIST do
        ...
        if idx >= top_idx and idx < top_idx + page_N then
            ...
            local xofs = 0
            if shifted_idx >= page_N1 then y = y - page_N1*dy; xofs = LCD_W/2 end

            lcd.drawText(edgeX+xofs, y, name, TEXT_COLOR)
            if p.typ < MBRIDGE_PARAM_TYPE_LIST then
                lcd.drawText(LCD_W/3+xofs, y, p.value.." "..p.unit, cur_attr_p(idx, pidx))
            elseif p.typ == MBRIDGE_PARAM_TYPE_LIST then
                lcd.drawText(LCD_W/3+xofs, y, p.options[p.value+1], cur_attr_p(idx, pidx))
            end
        end
...
local function doPageEdit(event, page_str)
    lcd.drawFilledRectangle(0, 0, LCD_W, titleHeight, TITLE_BGCOLOR)
...
local function drawPageTools()
    ...
    y = titleHeight+fontSize
    lcd.drawText(edgeX, y, "System Bootloader", cur_attr(Tools_Boot_idx))
end
...
local function drawPageMain()
    ...
    y = titleHeight
    lcd.drawText(edgeX, y, "Tx:", TEXT_COLOR)
    if DEVICE_ITEM_TX == nil then
        lcd.drawText(edgeX, y+fontSize, "---", TEXT_COLOR)
    else
        lcd.drawText(edgeX, y+fontSize, DEVICE_ITEM_TX.name, TEXT_COLOR+SMLSIZE)
        lcd.drawText(edgeX, y+fontSize+fontSizeSML, DEVICE_ITEM_TX.version_str, TEXT_COLOR+SMLSIZE)
        if DEVICE_INFO ~= nil then
            lcd.drawText(edgeX, y+fontSize+fontSizeSML*2, "ConfigId "..tostring(DEVICE_INFO.tx_config_id), TEXT_COLOR+SMLSIZE)
        end
    end

    lcd.drawText(LCD_W/2, y, "Rx:", TEXT_COLOR)
    if not DEVICE_PARAM_LIST_complete then
        -- don't do anything
    elseif not connected then
        lcd.drawText(LCD_W/2, y+fontSize, "not connected", TEXT_COLOR)
    elseif DEVICE_ITEM_RX == nil then
        lcd.drawText(LCD_W/2, y+fontSize, "---", TEXT_COLOR)
    else
        lcd.drawText(LCD_W/2, y+fontSize, DEVICE_ITEM_RX.name, TEXT_COLOR+SMLSIZE)
        lcd.drawText(LCD_W/2, y+fontSize+fontSizeSML, DEVICE_ITEM_RX.version_str, TEXT_COLOR+SMLSIZE)
    end
    ...
    y = y+fontSize+fontSizeSML*3
    lcd.drawText(edgeX, y, "Bind Phrase", TEXT_COLOR)
    if DEVICE_PARAM_LIST_complete then
        ...
        for i = 1,6 do
            ...
            if i == 6 and DEVICE_PARAM_LIST[2].value == 0 then -- do only for 2.4GHz band
                lcd.drawText(LCD_W/2, y, getExceptStrFromChar(c), TEXT_COLOR)
            end
        end
    end

    lcd.drawText(edgeX, y + fontSize, "Mode", TEXT_COLOR)
    if DEVICE_PARAM_LIST_complete then
        local p = DEVICE_PARAM_LIST[1] -- param_idx = 1 = Mode
        if p.options[p.value+1] ~= nil then
            lcd.drawText(LCD_W/3, y+fontSize, p.options[p.value+1], cur_attr_p(Mode_idx,1))
        end
    end

    lcd.drawText(edgeX, y + 2*fontSize, "RF Band", TEXT_COLOR)
    if DEVICE_PARAM_LIST_complete then
        ...
        if p.options[p.value+1] ~= nil then
            ...
            if p.value <= #freq_band_list then
                lcd.drawText(LCD_W/3, y + 2*fontSize, freq_band_list[p.value], cur_attr_p(RFBand_idx,2))
            else
                lcd.drawText(LCD_W/3, y + 2*fontSize, p.options[p.value+1], cur_attr_p(RFBand_idx,2))
            end
        end
    end

    if DEVICE_PARAM_LIST_complete and DEVICE_PARAM_LIST[3].allowed_mask > 0 then
        lcd.drawText(LCD_W*2/3, y, "Ortho", TEXT_COLOR)
        local p = DEVICE_PARAM_LIST[3] -- param_idx = 3 = RfOrtho
        if p.options[p.value+1] ~= nil then
            lcd.drawText(LCD_W-edgeX, y, p.options[p.value+1], cur_attr_p(RFOrtho_idx,3)+RIGHT)
        end
    end

    y = y + 3*fontSize
    lcd.drawText(edgeX, y, "Edit Tx", cur_attr(EditTx_idx))
    if not connected then
        lcd.drawText(edgeX + LCD_W/6, y, "Edit Rx", TEXT_DISABLE_COLOR)
    else
        lcd.drawText(edgeX + LCD_W/6, y, "Edit Rx", cur_attr(EditRx_idx))
    end
    lcd.drawText(edgeX + LCD_W*2/6, y, "Save", cur_attr(Save_idx))
    lcd.drawText(edgeX + LCD_W*3/6, y, "Reload", cur_attr(Reload_idx))
    lcd.drawText(edgeX + LCD_W*4/6, y, "Bind", cur_attr(Bind_idx))
    lcd.drawText(edgeX + LCD_W*5/6, y, "Tools", cur_attr(Tools_idx))

    -- show overview of some selected parameters
    y = y + fontSize
    lcd.setColor(CUSTOM_COLOR, GREY)
    lcd.drawFilledRectangle(0, y+fontSize/2, LCD_W, 1, CUSTOM_COLOR)

    y = y + fontSize
    --if not DEVICE_PARAM_LIST_complete then
    if DEVICE_DOWNLOAD_is_running then
        lcd.drawText(edgeX + LCD_W*2/6, y+fontSize, "parameters loading ...", TEXT_COLOR+BLINK+INVERS)
        return
    end

    lcd.drawText(edgeX, y, "Tx Power", TEXT_COLOR)
    lcd.drawText(edgeX, y+fontSize, "Tx Diversity", TEXT_COLOR)
    if DEVICE_INFO ~= nil then
        lcd.drawText(LCD_W/3, y, tostring(DEVICE_INFO.tx_power_dbm).." dBm", TEXT_COLOR)
        if DEVICE_INFO.tx_diversity <= #diversity_list then
            lcd.drawText(LCD_W/3, y+fontSize, diversity_list[DEVICE_INFO.tx_diversity], TEXT_COLOR)
        else
            lcd.drawText(LCD_W/3, y+fontSize, "?", TEXT_COLOR)
        end
    else
        lcd.drawText(LCD_W/3, y, "---", TEXT_COLOR)
        lcd.drawText(LCD_W/3, y+fontSize, "---", TEXT_COLOR)
    end
    ...
    lcd.drawText(edgeX+LCD_W/2, y, "Rx Power", rx_attr)
    lcd.drawText(edgeX+LCD_W/2, y+fontSize, "Rx Diversity", rx_attr)
    if DEVICE_INFO ~= nil and connected then
        lcd.drawText(LCD_W/3+LCD_W/2, y, tostring(DEVICE_INFO.rx_power_dbm).." dBm", rx_attr)
        if DEVICE_INFO.rx_diversity <= #diversity_list then
            lcd.drawText(LCD_W/3+LCD_W/2, y+fontSize, diversity_list[DEVICE_INFO.rx_diversity], rx_attr)
        else
            lcd.drawText(LCD_W/3+LCD_W/2, y+fontSize, "?", rx_attr)
        end
    else
        lcd.drawText(LCD_W/3+LCD_W/2, y, "---", rx_attr)
        lcd.drawText(LCD_W/3+LCD_W/2, y+fontSize, "---", rx_attr)
    end

    y = y + 2*fontSize
    lcd.drawText(edgeX, y, "Sensitivity", TEXT_COLOR)
    if DEVICE_INFO ~= nil then
        lcd.drawText(LCD_W/3, y, tostring(DEVICE_INFO.receiver_sensitivity).." dBm", TEXT_COLOR)
    else
        lcd.drawText(LCD_W/3, y, "---", TEXT_COLOR)
    end
...
local function doPageMain(event)
    ...
    lcd.drawText(LCD_W-1, 0, version, MENU_TITLE_COLOR+SMLSIZE+RIGHT)
